...
function updatePageContent(movie) {
  document.title = movie.name + " – Flix Nomad";
  const mainContents = `
          <div class="container m-top-100px mb-4">
            <div class="container">
              ...
              <h6 class="text-center movie-title-genre">
                <span>${movie.genre.genre1}</span><span>${movie.genre.genre2}</span><span>${movie.genre.genre3}</span>
              </h6>
              <div
                  ...
                  <div class="container">
                    <table class="table">
                      <tbody>
                        ...
                        <tr>
                          <td class="fw-bold">Rated:</td>
                          <td>${movie.rated}</td>
                        </tr>
                        <tr>
                          <td class="fw-bold">IMDb Link:</td>
                          <td>
                            <u>
                              <a
                                href="https://www.imdb.com/title/${movie.ratings.dataTitle}/?ref_=plg_rt_1"
            ...
            <div class="container">
              <div class="d-grid gap-2">
                <button class="btn btn-danger" type="button">
                  <a class="no-decoration text-light" href="${movie.downloadLink}"
...
function updateRelatedMovies(genre, selectedMovieId) {
  const relatedElement = document.getElementById("related");
  const moviesWithSameGenre = findMoviesByGenre(genre);
  const filteredMovies = moviesWithSameGenre.filter(
    (movie) => movie.id !== selectedMovieId
  );
  if (filteredMovies.length > 0) {
    const relatedContents = `<h2 class="single-pruduct-related">More to Watch</h2><ul>${filteredMovies
      .slice(0, 10)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (id) {
    ...
    if (movie) {
      ...
      updateRelatedMovies(movie.genre.genre1, movie.id); // Pass the selected movie ID
    } else {
      // Handle the case when the movie with the given id is not found
      const mainElement = document.querySelector("#js-movie-details");
      const notFoundElement = document.createElement("h2");
      notFoundElement.textContent = "Movie Unavailable";
      notFoundElement.classList.add("text-center", "py-4", "text-danger");
